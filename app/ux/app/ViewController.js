...
Ext.define('Ext.ux.app.ViewController', {
    ...
    msgWait: t('Wait...'),
    msgConfirmationDelete: t('Really delete the record(s) selected?'),
    ...
    msgConfirmation: t('confirmationDelete'),
    msgDeleteAll: t('confirmationDeleteAll'),
    ...
    onRenderModule: function() {
        ...
        me.updateLot = !App.user.isOperator;
    ...
    showHideFields: function(type) {
        ...
        me.formPanel.getForm().getFields().each(function(field) {
            ...
            if (type == 'edit') { //mostra todos os campos que estao com fieldsHideUpdateLot
    ...
    onSave: function() {
        ...
        if (!form.isValid()) {
            Ext.ux.Alert.alert(me.titleWarning, t(me.msgFormInvalid), 'warning');
            return;
        }
        me.saveButton.disable();
                            ...
                            listeners: {
                                toggle: function(btn, pressed) {
                                    if (btn.up('panel').down('#add').pressed || btn.up('panel').down('#remove').pressed) {
                                        field.setMask(pressed ? t('maskPercent') : t('maskMoney'));
                                    } else {
