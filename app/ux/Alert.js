...
Ext.define('Ext.ux.Alert', {
	...
	alert: function(title, msg, type, closable, autoHide, delay, icon, showIcon) {
		...
		tpl = ['<table class="alert-', type, '">',
					'<tbody>',
						'<tr>',
							'<td width="16px">', tagIcon, '</td>',
							'<td>', tagTitle, '</td>',
							'<td style="vertical-align: top">', tagClose, '</td>',
						'</tr>',
						'<tr>',
							'<td colspan="3"><p style="font-size: '+sizeContent+';">', content, '<p></td>',
						'</tr>',
					'</tbody>',
				'</table>'];
		...
		if(closable) {
			msgBox.el.down('.close').on('click', function() {
				msgBox.ghost('t', {remove: true});
			}, me);
		}

		//msgBox.hide();
		//msgBox.slideIn('t');
		autoHide && msgBox.ghost('t', {delay: delay || 3000, remove: true});
	}
},
