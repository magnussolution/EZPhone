Ext.define('Overrides.form.field.ComboBox', {
    ...
    initComponent: function() {
        ...
        me.store = Ext.isString(me.store) ? Ext.create('CallCenter.store.' + me.store) : me.store;
    ...
    filterStore: function(combo, evt) {
        ...
        {
            var valueFilter = Ext.encode([{type: 'string', value: combo.getValue(), field: combo.displayField}]);
            combo.store.proxy.extraParams.filter = valueFilter;
            me.deferredLoad.delay(2000, null, [{filter: valueFilter}]);
        }
    },
    onEditForm: function(form) {
        var me = this,
            value = form.getRecord().get(me.name),
            filter = Ext.encode([{type: 'list', value: [value], field: me.valueField}]);
